...
int main()
{
	...
	float bodywy = .5, bodyr = 1.2, bodyh = .4, bodyp = 1.1,
		  ...
		  fvwy = .22, fvr = 1.22, fvw = .7, fvwhalf = .05, fva1 = 40./180*pi, fva2 = 75./180*pi, fvp = 1.2, fvwpadratio = 1.3,
		  bgnwy = 1e-5;
	int fvn = 12, bgnn = 12;
	...
	Frame fm(fm_p_sec);
	fm.add_objs({new IntObj("bgn", true, {t_shine,FLT_MAX}), new Obj("bunny"), new Obj("piggy", {t_pig,FLT_MAX}), new IntObj("fvup", true, {t_3,t_shine}), new IntObj("fvdn", true, {t_3,t_shine}), new Obj("five", {t_shine,FLT_MAX})});
	...
	// object: five
	float fvwrec = .12, fvh = .25, fvlen = .42;
	Obj *pfv0u = new Rectangle("", {0, fvh}, fvlen, fvwrec),
		*pfv0c = new Rectangle("", {-(fvlen-fvwrec)/2., fvh/2.}, fvwrec, fvlen),
		*pfv0d = new EllArc("", {0, -fvwy+fvwhalf}, fvwy*fvr, fvwy, 0, fvw, -halfpi-fva2, halfpi+fva1),
		*pfv0up = pfv0u->copy(), *pfv0cp = pfv0c->copy(), *pfv0dp = pfv0d->copy();
	cout << "aaa" << endl;
	dynamic_cast<Rectangle*>(pfv0up->scale(fvp))->solid = false; dynamic_cast<Rectangle*>(pfv0up)->wid1 *= fvwpadratio; dynamic_cast<Rectangle*>(pfv0up)->wid2 *= fvwpadratio;
	dynamic_cast<Rectangle*>(pfv0cp->scale(fvp))->solid = false; dynamic_cast<Rectangle*>(pfv0cp)->wid1 *= fvwpadratio; dynamic_cast<Rectangle*>(pfv0cp)->wid2 *= fvwpadratio;
	dynamic_cast<EllArc*>(pfv0dp->scale(fvp))->solid = false; dynamic_cast<EllArc*>(pfv0dp)->width *= fvwpadratio;

	fm["fvup"].add_objs({new Obj("five"), new Obj("filter")});
	fm["fvdn"].add_objs({new Obj("five"), new Obj("filter")});
	fm["fvup"]["five"].add_objs({pfv0u->copy(), pfv0c->copy(), pfv0d->copy()});
	fm["fvdn"]["five"].add_objs({pfv0u->copy(), pfv0c->copy(), pfv0d->copy()});
	float filter_dx = 2*fvwy*fvr / fvn, filter_intsc = -fvwy*fvr;
	bool is_up = true;
	for(int i=0; i<fvn; ++i) {
		if(is_up) fm["fvup"]["filter"].add_objs({new Line("", 0, filter_intsc, filter_dx)});
		else fm["fvdn"]["filter"].add_objs({new Line("", 0, filter_intsc, filter_dx)});
		is_up = (!is_up); filter_intsc += filter_dx;
	}
	fm["five"].add_objs({pfv0up, pfv0cp, pfv0dp, pfv0u, pfv0c, pfv0d});
	...
	// motion: five
	float fvalpha = 10;
	Underdamp fvud(fvalpha, 5);
	fm["fvup"].add_motions({
		new Displace({t_3, t_shine}, [&](float t)->Fpair {return {0, (1+2*fvwy-fvwhalf)/(1-1/fvalpha)*(1-1/fvud(t))};}, Motion::Type::abspos) });
	fm["fvdn"].add_motions({
		new Displace({t_3, t_shine}, [&fm](float t)->Fpair {return -fm["fvup"].pc;}, Motion::Type::abspos)
	});
	...
	fm["five"].recenter({0,-2*fvwy+fvwhalf});
	fm["five"].add_motions({
		new Rotate({t_shake, FLT_MAX}, [&](float t) {return shake_amp*sin(2*pi/shake_period*t);}) });
